...
import { createHotelAging } from "../../_helpers/actions"
...
export function CreateHotelAgingModal({ hotelId }: Props) {
  ...
  const mutation = useMutation({
    mutationFn: (data: CreateMutation) => createHotelAging(hotelId, data.data),
    onSuccess: (data) =>
      showResponse(data, () => {
        if (data.status === 201) {
          form.reset()
  ...
  return (
    <Dialog open={modal} onOpenChange={setModal}>
      ...
      <DialogContent className='min-w-[800px] overflow-y-scroll'>
        <DialogHeader>
          <DialogTitle>
            Create Hotel Aging for Hotel - <b>{hotelId}</b>
          </DialogTitle>
        </DialogHeader>
        <Form {...form}>
