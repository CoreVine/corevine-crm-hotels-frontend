...
import { CitySchema, RateSchema } from "@/schema";
...
import { createRate } from "../_helpers/actions";

export const CreateRateModal = () => {
  const [modal, setModal] = useState(false);

  const form = useForm({
    resolver: zodResolver(RateSchema.Create)
  });

  const mutation = useMutation({
    mutationFn: (data: z.infer<typeof RateSchema.Create>) => createRate(data),
    onSuccess: (data) =>
      showResponse(data, () => {
        if (data.status === 201) {
          setModal(false);
